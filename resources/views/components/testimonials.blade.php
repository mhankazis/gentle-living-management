...
<section class="py-16 bg-gradient-to-r from-blue-600 to-purple-600">
  <div class="container mx-auto px-4">
    <div class="text-center mb-12">
      <h2 class="text-4xl font-bold text-white mb-4">Cerita para Ibu</h2>
    </div>
    ...
    class="relative max-w-6xl mx-auto">
      ...
      <!-- Navigation Arrows -->
      <button @click="prevSlide()" class="absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-4 z-10 bg-white/20 hover:bg-white/30 text-white p-3 rounded-full transition-all duration-300 backdrop-blur-sm">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
        </svg>
      </button>
      
      <button @click="nextSlide()" class="absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-4 z-10 bg-white/20 hover:bg-white/30 text-white p-3 rounded-full transition-all duration-300 backdrop-blur-sm">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <template x-for="(testimonial, index) in testimonials.slice(0, 3)" :key="index">
                <div class="bg-white rounded-lg p-6 shadow-lg transform hover:scale-105 transition-transform duration-300">
                  <div class="flex items-start space-x-4">
                    <div class="w-16 h-16 bg-gradient-to-r from-blue-400 to-purple-400 rounded-full flex-shrink-0 flex items-center justify-center">
                      <span class="text-white font-bold text-xl" x-text="testimonial.author.split(' ')[1].charAt(0)"></span>
                    </div>
                    <div class="flex-1">
                      <p class="text-gray-600 mb-4 leading-relaxed text-sm" x-text="testimonial.text"></p>
                      <h4 class="font-semibold text-blue-600" x-text="testimonial.author"></h4>
                    </div>
                  </div>
                </div>
              </template>
          ...
          <div class="w-full flex-shrink-0">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 px-4">
              <template x-for="(testimonial, index) in testimonials.slice(3, 6)" :key="index">
                <div class="bg-white rounded-lg p-6 shadow-lg transform hover:scale-105 transition-transform duration-300">
                  <div class="flex items-start space-x-4">
                    <div class="w-16 h-16 bg-gradient-to-r from-purple-400 to-pink-400 rounded-full flex-shrink-0 flex items-center justify-center">
                      <span class="text-white font-bold text-xl" x-text="testimonial.author.split(' ')[1].charAt(0)"></span>
                    </div>
                    <div class="flex-1">
                      <p class="text-gray-600 mb-4 leading-relaxed text-sm" x-text="testimonial.text"></p>
      ...
      <div class="mt-6 max-w-md mx-auto">
        <div class="w-full bg-white/20 rounded-full h-1">
          <div class="bg-white h-1 rounded-full transition-all duration-4000 ease-linear" 
